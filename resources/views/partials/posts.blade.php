    ...
    <div class="flex space-x-2">
        <form action="{{ route('vote', $post) }}" method="POST" >
            ...
            <div class="flex flex-col items-center">
                <button type="submit" name="vote" value="upvote" class="{{ Auth::user()->votes->where('vote_type', 'upvote')->contains('post_id', $post->id) ? 'text-green-500' : 'text-gray-200' }}">
                ...
                <button type="submit" name="vote" value="downvote" class="{{ Auth::user()->votes->where('vote_type', 'downvote')->contains('post_id', $post->id) ? 'text-red-500' : 'text-gray-200' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
